...
void NetworkInterface::recv_frame( const EthernetFrame& frame )
{
  ...
  if ( parse( arpMessage, frame.payload ) ) {
    ...
    if (arpMessage.opcode == ARPMessage::OPCODE_REQUEST) {
      ...
      mp_[arpMessage.sender_ip_address] = std::pair<size_t, EthernetAddress>(cur_time, arpMessage.sender_ethernet_address);
      ...
      // learn from sender to update ip_address
      mp_.emplace(arpMessage.sender_ip_address, std::pair<size_t, EthernetAddress>(cur_time, arpMessage.sender_ethernet_address));
